...
We can create a logistic regression model to 
...
Of the coefficients for the days of the week, Tuesday is the only one whose range does not include zero. This means that, based on our model, it is significantly likely that Tuesday has more rain than other days. 

When we add the coefficent for a day to the incercept, we get the log odds that it will rain on that day, based solely on the day of the week. Friday is the reference day (because it's the first alphabetically), so the intercept represents the log odds of rain on Fridays. 

To find probability of rain on Tuesday:

```{r}

intercept <- as.numeric(fit_table$estimate[fit_table$term == "(Intercept)"])
tues_coef <- as.numeric(fit_table$estimate[fit_table$term == "day_of_weekTue"])
