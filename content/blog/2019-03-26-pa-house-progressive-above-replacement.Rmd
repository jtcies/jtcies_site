...
election_2016_url <- "https://en.wikipedia.org/wiki/2016_Pennsylvania_House_of_Representatives_election"

election_results <- election_2016_url %>%
    read_html() %>%
    html_table(fill = TRUE) %>%
    .[[19]] %>%
    janitor::clean_names() %>%
    tbl_df() %>%
    select(-party, -party_2, -party_3) %>%
...
election_tidy <- election_results %>%
    ...
    mutate(
		...
		margin = percent - lead(percent, default = 0)
	) %>%
	filter(win)
...
election_merge <- election_tidy %>%
	separate(candidate, into = c("first", "last", "suffix"), sep = " |-") %>%
...
pa_house_tidy %>%
  group_by(party) %>% 
  summarise(
    `count uncontested` = sum(margin == 100),
    `percent uncontested` = percent(mean(margin == 100))
  ) %>% 
  knitr::kable()
...
par_mod <- glm(
	progressive_avg ~ margin:party, 
	data = pa_house_tidy,
	family = "binomial",
	weights = votes_cast
)

augment(par_mod, type.predict = "response")	%>%
	ggplot(aes(x = margin, color = party)) +
