...
Interestingly, one uncontested Democratic 2016 district flipped to Republican in 2018. That was District 48, where Tim O'Neal won via special election in early 2018, and the re-won election by 11.5 points later in the general later in the year. 

```{r}

pa_house_tidy %>%
    filter(party_2016 == "Democratic", margin_2016 == 100,
           party_2018 == "Republican") %>%
    select(district, representative_2016, representative_2018, margin_2018)
...
prog_plot <- augmented %>%
	ggplot(aes(x = margin_2016, color = party_2016, group = party_2016,
    ...
    geom_point(
      ...
      color = "black"
		...
		labs(
            ...
            subtitle = "Outlined points show representatives re-elected in 2018",
			x = "margin of victory - 2016 election",
			y = "progressive avg",
			color = ""	
		)
...
prog_plotly <- ggplotly(prog_plot, 
                        tooltip = c("x", "y", "text", "relected", "rep"))
