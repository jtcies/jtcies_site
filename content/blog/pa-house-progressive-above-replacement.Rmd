...
prog_plot <- augmented %>%
    ...
    annotate("rect",
        xmin = 0, xmax = 31, ymin = .33, ymax = .65,
...
The part of the chart that stands out me is the Republicans who had a margin of victory below 50 points. The table below shows the 2018 outcome comparing Repulicans who had a higher progressive average than expected compared to those who voted less progressive than expected.

```{r}

 augmented %>% 
     filter(party_2016 == "Republican", margin_2016 < 50) %>%
     group_by(negative = .fitted < progressive_avg) %>%
     count(status_2018, party_2018)
```
...
Let's take a look at two groups: Democrats in uncontested voes who are less progressive than expected. 
...
There are `r nrow(dems_less_progressive)` Democrats who won uncontested in 2016 races who were less progressive than expected. Let's look at the top 10. 
...
According to this model, Chris Sainto and Tony DeLuca won without a challenge to their seat.  
...
I set out to replicate the methodology used by Data for Progress. While not a perfect replicaiton, this analysis can tell something about voting behavior in relation to the level of partisanship in a particular district. 

However, the most interesting thing to me here is the way in which Republicans who showwed any level of progressiveness were decimated in 2018. Perhaps this is related to the 'blue wave', and most of those vulnerable seats flipped; perhaps this is 
