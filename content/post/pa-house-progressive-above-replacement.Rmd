...
pa_house_tidy <- pa_house_tidy %>%
    ...
    mutate(
        margin_scaled = scale(margin_2016),
        party_int = as.integer(as.factor(party_2016))
    )

model_dat <- list(
    votes = pa_house_tidy$votes_cast,
    progressive_votes = as.integer(pa_house_tidy$progressive_votes),
    party_int = pa_house_tidy$party_int,
    margin_scaled = pa_house_tidy$margin_scaled
)
...
glmod <- ulam(
    alist(
        progressive_votes ~ binomial(votes, p),
        logit(p) <- a_mu[1] + a[party_int, 1] + (a_mu[2] + a[party_int, 2]) * margin_scaled,
        matrix[party_int, 2]: a <- t(diag_pre_multiply(sigma, L_Rho) * z),
        matrix[2, party_int]: z ~ normal(0, 1),
        vector[2]: a_mu[[1]] ~ normal(0, 1),
        vector[2]: a_mu[[2]] ~ normal(0, 1),
...
sim_dat <- crossing(
    party_int = 1:2,
    margin_scaled = seq(-3, 3, length.out = 0.1)
)

post_sim <- link(glmod, sim_dat)
