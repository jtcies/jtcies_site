...
sim_dat <- crossing(party_2016 = c("Democratic", "Republican"),
                    margin_2016 = seq(0, 100, length.out = 100))

simulations <- sim_dat %>%
    nest(data = everything()) %>%
    crossing(model_fit = sample_fits$model_fit) %>%
    mutate(
        augmented = map2(model_fit, data, 
...
prog_plot <- pa_house_tidy %>%
    ggplot(aes(margin_2016, progressive_avg, color = party_2016)) +
        geom_point(
...
ggplotly(prog_plot, tooltip = "district")
