...
cox_fit <- coxph(Surv(minuteGame, gt) ~ poly(marginScore, 3), data = surv_df)
...
cox_df <- as_tibble(cox_pred$surv)
...
surv_preds %>%
    mutate(marginScore = fct_inorder(marginScore)) %>%
    ggplot(aes(minuteGame, surv_prob, color = marginScore)) +
        geom_line() +
        scale_y_continuous(limits = c(0, NA), labels = scales::percent) +
        labs(
            title = "Surivival curves for garbage time based on Cox Hazard model of score margins",
            y = "probability of not entering garbage time",
            x = "minute in game",
            color = "score margin"
        )
...
win_preds %>%
    ...
    ggplot(aes(minuteGame, .fitted, color = factor(margin), linetype = team)) +
        geom_line(size = 1.5) +
