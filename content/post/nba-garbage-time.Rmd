...
minute <- seq(36, 48, by = 0.1)

beta_sim <- rnorm(N, .5, .02)

alpha <- rnorm(N, 10, 2)

crossing(beta_sim, alpha, minute) %>% 
    mutate(margin = alpha - exp(beta_sim * minute)) %>% 
    group_by(minute) %>% 
...
gt_start_list <- list(
    minute = garbage_time_start$minuteGame,
    margin = abs(garbage_time_start$marginScore)
)

gt_mod <- ulam(
    alist(
        margin ~ lognormal(mu, sigma),
        mu <- a + exp(b * minute),
        a ~ normal(10, 2),
        b ~ normal(.5, 0.2),
        sigma ~ exponential(1)
...
preds <- exp(link(gt_mod, data = sim_dat)) %>% 
    t() %>%
    as_tibble() %>% 
    bind_cols(sim_dat) %>% 
...
fourth_qtr_w_final %>%
    filter(winning_team_ahead) %>%
    group_by(idGame) %>%
    filter(idGame == min(idGame)) %>%
    group_by(minuteGame) %>%
    summarise(avg_margin = mean(abs(marginScore))) %>%
    ggplot(aes(minuteGame, avg_margin)) +
        geom_line()
