...
gt <- no_ot %>%
    mutate(
        ...
        # just time into fourth quarter
        minuteGame = minuteGame - 36
    ) 
...
cox_fit <- coxph(Surv(minuteGame, gt) ~ marginScore, data = all_gt)
...
cox_sim <- crossing(
  minuteGame = seq(36, 48, length.out = 100),
...
cox_sim %>% 
  mutate(surv = apply(cox_pred$surv, 2, median)) %>% 
  ggplot(aes(minuteGame, surv, color = factor(marginScore))) +
    geom_line()
...
preds %>% 
    ...
    ggplot(aes(minute, mean)) +
        ...
        labs(
            ...
            subtitle = "Based on when 10th player enters game in fourth quarter",
